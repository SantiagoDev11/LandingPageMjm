...
export default function TrabajaConNosotrosPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 flex flex-col">
      {/* Botón desactivado para esta página */}
        ...
        >
          ...
          <form ref={form} onSubmit={sendEmail} className="space-y-6">
            <div className="space-y-2">
              ...
              <input name="user_name" type="text" required className="w-full p-4 bg-slate-50 border rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 text-slate-900" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <input name="user_email" type="email" required className="w-full p-4 bg-slate-50 border rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 text-slate-900" />
            </div>
            ...
            <div className="space-y-2 text-center">
              ...
              <div className="border-2 border-dashed border-blue-200 rounded-3xl p-10 hover:bg-blue-50 transition-all relative">
                <input type="file" name="mi_archivo" accept=".pdf" required onChange={handleFileChange} className="absolute inset-0 opacity-0 cursor-pointer" />
                <div className="text-blue-600 font-semibold italic">
                  {fileName ? <span className="text-green-600 font-bold">✅ {fileName}</span> : "Haz clic para subir tu PDF"}
                </div>
              </div>
            </div>

            {isSent && (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-green-600 font-bold text-center bg-green-50 py-2 rounded-xl">
                ¡Postulación enviada correctamente!
              </motion.p>
            )}

            <button disabled={loading} className="w-full bg-[#0d2a61] text-white py-5 rounded-full font-bold uppercase tracking-widest hover:bg-blue-900 transition-all shadow-lg disabled:opacity-50">
